...
#pragma warning restore 0649
    ...
    public string Name
    {
        ...
        set
        {
            ...
            {
                throw new Exception(SR.Format(SR.net_cookie_attribute, "Name", value ?? "<null>"));
            }
        }
    }
    ...
    internal bool VerifySetDefaults(CookieVariant variant, Uri uri, bool isLocalDomain, string localDomain, bool setDefault, bool shouldThrow)
    {
            ...
            m_name.AsSpan().ContainsAny(s_reservedToNameChars))
        {
            if (shouldThrow)
            {
                throw new Exception(SR.Format(SR.net_cookie_attribute, "Name", m_name ?? "<null>"));
            }
            return false;
        }
            ...
            (!(m_value.Length > 2 && m_value.StartsWith('\"') && m_value.EndsWith('\"')) && m_value.AsSpan().ContainsAny(';', ',')))
        {
            if (shouldThrow)
            {
                throw new Exception(SR.Format(SR.net_cookie_attribute, "Value", m_value ?? "<null>"));
            }
            return false;
        }
            ...
            && (Comment.AsSpan().ContainsAny(';', ',')))
        {
            if (shouldThrow)
            {
                throw new Exception(SR.Format(SR.net_cookie_attribute, CookieFields.CommentAttributeName, Comment));
            }
            return false;
        }
            ...
            && (Path.AsSpan().ContainsAny(';', ',')))
        {
            if (shouldThrow)
            {
                throw new Exception(SR.Format(SR.net_cookie_attribute, CookieFields.PathAttributeName, Path));
            }
            return false;
        }
        ...
        else
        {
            if (!m_domain_implicit)
            {
                ...
                if (!DomainCharsTest(domain))
                {
                    if (shouldThrow)
                    {
                        throw new Exception(SR.Format(SR.net_cookie_attribute, CookieFields.DomainAttributeName, domain ?? "<null>"));
                    }
                    return false;
                }
            ...
            if (!valid)
            {
                if (shouldThrow)
                {
                    throw new Exception(SR.Format(SR.net_cookie_attribute, CookieFields.DomainAttributeName, m_domain));
                }
                return false;
            }
        }
        ...
        if (m_port_implicit == false)
        {
            ...
            if (!valid)
            {
                if (shouldThrow)
                {
                    throw new Exception(SR.Format(SR.net_cookie_attribute, CookieFields.PortAttributeName, m_port));
                }
                return false;
            }
        }
    ...
    public string Port
    {
        ...
        set
        {
            ...
            else
            {
                ...
                {
                    throw new Exception(SR.Format(SR.net_cookie_attribute, CookieFields.PortAttributeName, value));
                }
