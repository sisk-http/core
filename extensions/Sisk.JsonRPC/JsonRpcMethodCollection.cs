...
public sealed class JsonRpcMethodCollection
{
    ...
    public void AddMethodsFromType<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicMethods)] T>(T target, bool prefixTypes) where T : notnull
    {
        lock ((methods as ICollection).SyncRoot)
        {
    ...
    /// <param name="type">The type from which to scan and add methods.</param>
    /// <param name="target">The target object instance containing the methods.</param>
    public void AddMethodsFromType(Type type, object? target) => AddMethodsFromType(type, target, false);

    /// <summary>
    /// Adds methods from the specified type to the collection without prefixing method names.
    /// </summary>
    /// <param name="type">The type from which to scan and add methods.</param>
    public void AddMethodsFromType(Type type) => AddMethodsFromType(type, null, false);
